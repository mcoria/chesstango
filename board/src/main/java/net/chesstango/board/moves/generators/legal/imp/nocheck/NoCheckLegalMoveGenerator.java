...
public class NoCheckLegalMoveGenerator extends AbstractLegalMoveGenerator {
    ...
    @Override
    public MoveContainerReader<MoveImp> getLegalMoves(AnalyzerResult analysis) {
        ...
        MoveContainer<MoveImp> moves = new MoveContainer<>(Long.bitCount(safePositions));
    ...
    protected void getLegalMovesNotKingNotPinned(long safePositions, MoveContainer<MoveImp> moves) {
        ...
        for (Iterator<PiecePositioned> iterator = positionReader.iterator(safePositions); iterator.hasNext(); ) {
            ...
            MoveList<MoveImp> pseudoMoves = getPseudoMoves(origen);
    ...
    protected void getLegalMovesNotKingPinned(List<AbstractMap.SimpleImmutableEntry<PiecePositioned, Cardinal>> pinnedPositionCardinals, MoveContainer<MoveImp> moves) {
        for (AbstractMap.SimpleImmutableEntry<PiecePositioned, Cardinal> pinnedPositionCardinal : pinnedPositionCardinals) {
            ...
            MoveList<MoveImp> pseudoMoves = getPseudoMoves(from);
    ...
    protected void getLegalMovesKing(long safeKingPositions, MoveContainer<MoveImp> moves) {
        Square kingSquare = getCurrentKingSquare();

        MoveList<MoveImp> pseudoMovesKing = getPseudoMoves(kingSquare);
    ...
    protected void getCastlingMoves(MoveContainer<MoveImp> moves) {
